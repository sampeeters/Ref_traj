...
ggplot() +
  geom_path(data = filter(Traj_data, FLIGHT_ID==Flight_id), aes(x=EVENT_TIME, y=ALT)) +
...
ggplot() + 
  ...
  geom_path(data = filter(Traj_data, FLIGHT_ID==Flight_id), aes(x=LON, y=LAT, colour=InRadius)) +
  geom_point(data = filter(Traj_data, FLIGHT_ID==Flight_id), aes(x=LON, y=LAT, colour=InRadius)) +
  ...
  labs(x="Longitude (°)", y="Latitude (°)", title=paste0("FR24 data for FLIGHT_ID ", Flight_id)) +
